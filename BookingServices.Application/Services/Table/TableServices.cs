...
public class TableServices : ITableServices
{
    ...
    public async Task<TableDTO> GetTableByIdAsync(Guid id) => _mapper.Map<TableDTO>(await _context.Tables.IgnoreQueryFilters().Include(x => x.Bookings.Skip(0).Take(10)).FirstOrDefaultAsync(x => x.Id == id));
