...
public class BookingServices : IBookingServices
{
    ...
    public async Task AddBookingAsync(AddBookingRequest booking)
    {
        ...
        //check table valid
        var validTable = _mapper.Map<TableDTO>(_bookingDbContext.Tables.Where(x => x.Id == booking.TableId).Include(x => x.Bookings.Where(x=> x.BookingDate.Date == booking.BookingDate.Date)).FirstOrDefault())?.IsAvailable()??false;
    ...
    public async Task<ApiPaged<BookingDTO>> GetAllBookingAsync(GetAllBookingRequest request)
    {
        ...
        return new ApiPaged<BookingDTO>
        {
            ...
            TotalRecords = await _bookingDbContext.Bookings.Include(x => x.Tables)
                    .WhereIf(request.UserId != null, x => x.CreatedBy == request.UserId)
                    .WhereIf(request.CustomerId != null, x => x.CustomerId == request.CustomerId)
                    .WhereIf(request.TableId != null, x => x.Tables.Where(x => x.Id == request.TableId).Any()).CountAsync()
        };
    }
